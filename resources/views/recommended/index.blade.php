...
@section('title', "Recommended Manga — Page {$page} — MangAlfa")

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 animate-fade-in">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            <h1 class="font-display font-700 text-2xl text-white">Recommended</h1>
            <span class="badge-new text-white text-xs px-2 py-1">For You</span>
        </div>
        <div class="flex items-center gap-3">
        ...
        <a href="{{ route('manga.show', $slug) }}" class="manga-card group block">
            <div class="relative rounded-xl overflow-hidden bg-dark-800 aspect-[2/3] mb-2">
                ...
                <div class="absolute top-1.5 right-1.5">
                    <span class="text-xs px-1.5 py-0.5 bg-black/60 rounded text-slate-300 text-[10px]">{{ $item['type'] }}</span>
                </div>
                @endif
    ...
    <div class="flex items-center justify-center gap-2">
        @if($page > 1)
        <a href="{{ route('recommended.index', ['page' => $page - 1, 'per_page' => ($perPage ?? 24)]) }}" class="px-4 py-2 bg-dark-800 hover:bg-dark-700 border border-dark-600 hover:border-accent/40 rounded-xl text-sm text-slate-300 hover:text-white transition-all flex items-center gap-2">
    ...
    <div class="text-center py-20">
        <p class="text-slate-500">No manga found. The API may be temporarily unavailable.</p>
